...
const EditProjectModal: React.FC<EditProjectModalProps> = ({ 
  ...
  const InputField: React.FC<{label: string, name: keyof ProjectFormData, value: string | number, error?: string, type?: string, isTextarea?: boolean, placeholder?: string, required?: boolean, list?: string}> = 
    ...
    <div className="relative">
      <label htmlFor={`edit-${name}`} className="block text-xs font-black text-gray-500 uppercase tracking-widest mb-1.5">
      ...
      <div className="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div className="inline-block align-bottom bg-white rounded-2xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-2xl w-full">
          ...
          <form onSubmit={handleSubmit} className="px-6 py-6 space-y-5">
            <InputField label="Project Name" name="projectName" value={formData.projectName} error={errors.projectName} required />
            <InputField label="Project Description" name="projectDescription" value={formData.projectDescription} isTextarea />
            
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-black text-gray-500 uppercase tracking-widest mb-1.5">Project Stage</label>
                <select name="projectStage" value={formData.projectStage} onChange={handleChange} className="block w-full rounded-xl border border-gray-200 p-3 text-sm focus:border-primary focus:ring-primary">
                  {PROJECT_STAGE_OPTIONS.map(opt => <option key={opt.value} value={opt.value}>{opt.label}</option>)}
                </select>
              </div>
              <div>
                <label className="block text-xs font-black text-gray-500 uppercase tracking-widest mb-1.5">Project Location</label>
                <select name="projectLocation" value={formData.projectLocation} onChange={handleChange} className="block w-full rounded-xl border border-gray-200 p-3 text-sm focus:border-primary focus:ring-primary">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <InputField label="Project Sector" name="projectSector" value={formData.projectSector} error={errors.projectSector} required list="edit-sector-suggestions" />
              <datalist id="edit-sector-suggestions">
                {sectorSuggestions.map(s => <option key={s} value={s} />)}
              </datalist>
              <InputField label="Sub-Location" name="projectSubLocation" value={formData.projectSubLocation} />
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <InputField label="Jobs (Target)" name="jobsToBeCreated" value={formData.jobsToBeCreated} type="number" />
              <InputField label="Worth ($)" name="investmentWorth" value={formData.investmentWorth} type="number" />
            </div>

            <div className="flex items-center space-x-3 bg-nasida-green-900/[0.03] p-4 rounded-xl border border-nasida-green-900/10">
              <input type="checkbox" id="edit-requiresFollowUp" name="requiresFollowUp" checked={formData.requiresFollowUp} onChange={handleChange} className="h-5 w-5 text-nasida-green-900 border-gray-300 rounded focus:ring-primary" />
              <label htmlFor="edit-requiresFollowUp" className="text-sm font-black text-gray-700 uppercase tracking-tight flex items-center">
            ...
            <div className="border-t border-gray-100 pt-5 space-y-4">
              ...
              <InputField label="Contact Name" name="focalPersonName" value={formData.focalPersonName} error={errors.focalPersonName} required />
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <InputField label="Phone" name="focalPersonPhone" value={formData.focalPersonPhone} type="tel" />
                <InputField label="Email" name="focalPersonEmail" value={formData.focalPersonEmail} type="email" />
              </div>
            </div>
